...
export default function DriverDashboard({user}: DriverDashboardProps) {
        ...
        if (latestRequest?.id !== rideRequest?.id) {
          ...
          if (latestRequest) {
            toast({
              ...
              description: `From: ${latestRequest.pickupAddress || 'Near specified location'}`,
            });
          }
  ...
  }, [driverData?.isAvailable, user.uid, rideRequest?.status]);
  ...
  if (loading || !driverData) {
    // Show loading if loading state is true OR driverData is not yet available
    return (
  ...
  if (error && driverData) {
    // Show error overlay if driver data *is* loaded but there's another issue
          ...
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3 pt-4 px-4 bg-card">
            <div className="flex items-center gap-3">
              <Avatar className="h-12 w-12 border-2 border-primary/20">
                ...
                <AvatarImage
                  src={`https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(driverData.name)}`}
                  alt={driverData.name}
                />
            ...
            <div className="flex items-center space-x-2">
              <Label
                ...
                className={`text-sm font-medium transition-colors ${
                  driverData.isAvailable ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'
                }`}
              >
          ...
          <CardContent className="px-4 pb-4 pt-2 bg-card">
            <div className="text-xs text-muted-foreground flex items-center gap-1">
              ...
              {currentLocation
                ...
                : 'Tracking...'}
              {driverData.isAvailable && !currentLocation && (
                <span className="text-yellow-600 ml-1">(Waiting for location)</span>
              )}
            ...
            <Card className="border-accent shadow-lg animate-pulse-border">
              <CardHeader className="pb-2 pt-3">
                <CardTitle className="flex items-center gap-2 text-accent text-lg">
                  <PhoneCall className="h-5 w-5" /> New Ride Request!
                </CardTitle>
              ...
              <CardContent className="space-y-1 pb-3">
                ...
                <p>
                  ...
                  {rideRequest.pickupAddress ||
                    `Near (${rideRequest.pickupLocation.latitude.toFixed(3)}, ${rideRequest.pickupLocation.longitude.toFixed(3)})`}
                </p>
                <p>
                  <span className="font-medium">To:</span> {rideRequest.destinationAddress || 'Not specified'}
                </p>
                <p>
                  <span className="font-medium">Rider:</span> {rideRequest.riderName || 'Customer'}
                </p>
              </CardContent>
              <CardFooter className="flex justify-end gap-3 pb-3 pt-0">
                <Button
                  variant="outline"
                  onClick={() => handleDeclineRequest(rideRequest.id)}
                  disabled={isProcessingRequest}
                >
                  <XCircle className="mr-1 h-4 w-4" /> Decline
                </Button>
                <Button
                  onClick={() => handleAcceptRequest(rideRequest.id)}
                  disabled={isProcessingRequest}
              ...
              <Card className="border-green-500 dark:border-green-400 shadow-md">
                <CardHeader className="pb-2 pt-3">
                  <CardTitle className="flex items-center gap-2 text-green-600 dark:text-green-400 text-lg">
                    <Car className="h-5 w-5" /> Ride In Progress
                  </CardTitle>
                ...
                <CardContent className="space-y-1 pb-3">
                  <p>
                    ...
                    {rideRequest.pickupAddress ||
                      `(${rideRequest.pickupLocation.latitude.toFixed(4)}, ${rideRequest.pickupLocation.longitude.toFixed(4)})`}
                  </p>
                  <p>
                    <span className="font-medium">Destination:</span> {rideRequest.destinationAddress || 'Not specified'}
                  </p>
                  <p>
                    <span className="font-medium">Rider:</span> {rideRequest.riderName || 'Customer'}
                  </p>
                  {rideRequest.riderPhone && (
                    <p>
                      ...
                      <a href={`tel:${rideRequest.riderPhone}`} className="text-accent underline hover:text-accent/80">
                        {rideRequest.riderPhone}
                      </a>
                    </p>
                  )}
                  ...
                  <Button
                    ...
                    onClick={() =>
                      window.open(
                        `https://www.google.com/maps/dir/?api=1&destination=${rideRequest.pickupLocation.latitude},${rideRequest.pickupLocation.longitude}`,
                        '_blank'
                      )
                    }
                    className="mt-2"
                ...
                <CardFooter className="flex justify-end gap-3 pb-3 pt-0">
                  ...
                  <Button
                    onClick={() => handleCompleteRide(rideRequest.id)}
                    disabled={isProcessingRequest}
            ...
            <Card className="bg-muted/30 shadow-inner rounded-lg">
              <CardContent className="pt-6 pb-6">
                <p className="text-center text-muted-foreground">
                  You are currently offline. Toggle the switch above to become available.
                </p>
              </CardContent>
            </Card>
          )}
