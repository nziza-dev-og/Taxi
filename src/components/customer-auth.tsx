...
export default function CustomerAuth() {
  ...
  const onRegisterSubmit = async (data: RegistrationFormData) => {
    ...
    try {
      ...
      // Save customer info to 'customers' collection in Firestore
      await setDoc(doc(db, 'customers', user.uid), {
        uid: user.uid,
        email: data.email,
        name: data.name,
        phone: data.phone || null, // Store phone or null if empty
      ...
      setActiveTab("login"); // Switch to login tab after successful registration
       ...
       if (err instanceof Error && 'code' in err) {
           ...
           setError('An unexpected error occurred during customer registration.');
           console.error(err);
       }
    } finally {
      setLoading(false);
  ...
  const onLoginSubmit = async (data: LoginFormData) => {
    ...
    try {
       ...
       // Verify if the logged-in user exists in the 'customers' collection
       const customerDocRef = doc(db, 'customers', user.uid);
       const customerDocSnap = await getDoc(customerDocRef);

       if (!customerDocSnap.exists()) {
            // If the user exists in Auth but NOT in the 'customers' collection
            await auth.signOut(); // Log them out immediately
            setError('Access Denied. This account is not registered as a customer.');
            toast({
                title: "Login Failed",
                description: "Could not find a customer account with these credentials.",
                variant: "destructive"
            })
      ...
      // User is authenticated AND verified as a customer in Firestore
      toast({
        title: "Login Successful",
        description: "Welcome back!",
      });
       loginForm.reset(); // Clear the login form
      // Let the listener in customer/page.tsx redirect
    } catch (err) {
       if (err instanceof Error && 'code' in err) {
           setError(handleFirebaseAuthError(err as AuthError));
       } else {
            setError('An unexpected error occurred during customer login.');
           console.error(err);
       }
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full max-w-md">
        ...
        <TabsContent value="login">
          <Card className="shadow-md rounded-lg">
            <CardHeader>
              <CardTitle className="text-2xl font-bold text-center flex items-center justify-center gap-2"> <User className="h-6 w-6"/> Customer Login</CardTitle>
              <CardDescription className="text-center text-muted-foreground">Log in to book your ride.</CardDescription>
            </CardHeader>
        ...
        <TabsContent value="register">
          <Card className="shadow-md rounded-lg">
            <CardHeader>
              <CardTitle className="text-2xl font-bold text-center flex items-center justify-center gap-2"><User className="h-6 w-6"/> Customer Registration</CardTitle>
              <CardDescription className="text-center text-muted-foreground">Create an account to start booking rides.</CardDescription>
            </CardHeader>
